...
public class Print implements IntVisitor {
    ...
    private void sayln(String  s) { out.println(s); }
    private void say  (String  s) { out.print(s);   }

    public void visit(SEQ s, int d) {
	indent(d); sayln("SEQ("); s.left.accept(this, d+1); sayln(",");
	s.right.accept(this, d+1); say(")");
    }
    public void visit(LABEL s, int d) {
	indent(d); say("LABEL "); say(s.label.toString());
    }
    public void visit(JUMP s, int d) {
	indent(d); sayln("JUMP("); s.exp.accept(this, d+1); say(")");
    }
	...
	sayln(","); s.left.accept(this, d+1); sayln(",");
	s.right.accept(this, d+1); sayln(",");
	indent(d+1); say(s.iftrue.toString()); say(","); 
	say(s.iffalse.toString()); say(")");
    }
    public void visit(MOVE s, int d) {
	indent(d); sayln("MOVE("); s.dst.accept(this,d+1); sayln(","); 
	s.src.accept(this, d+1); say(")");
    }
    public void visit(EXP s, int d) {
	indent(d); sayln("EXP("); s.exp.accept(this, d+1); say(")"); 
    }
	...
	sayln(",");
	e.left.accept(this, d+1); sayln(",");
	e.right.accept(this, d+1); say(")");
    }
    public void visit(MEM e, int d) {
	indent(d);
	sayln("MEM("); e.exp.accept(this, d+1); say(")");
    }
	...
	indent(d); sayln("ESEQ("); e.stm.accept(this, d+1); sayln(",");
	e.exp.accept(this, d+1); say(")");
    }
	...
	for (Iterator<Exp> args = ExpList.toArrayList(e.args).iterator(); args.hasNext(); ) {
	    sayln(","); args.next().accept(this, d+1);
	}
        say(")");
    }
}
