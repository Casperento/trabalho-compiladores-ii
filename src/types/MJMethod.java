...
public class MJMethod extends MJType {
    ...
    public String InsertLocal(String p_name, MJType p_type) {
        boolean error = Repeated(p_name) ;
        if (error)
            return "Local variable double declaration " + p_name + " in method " + name ;
    ...
    public void CheckParamList(ParamList list_p) {
        ...
        int sz2 = p_list.size();
        if (sz1 != sz2) {
            System.out.println("Error: number of parameters does not match in method " + name);
            return ;
        }
        ...
        for(int i = 0 ; i < sz1 ; i++) {
            ...
            if (!ptype2.CheckType(ptype1)) {
                System.out.println("Error: Imcompatible types for parameter " + p_name);
            }
        }
    }
    ...
    public MJType SearchType(String p_name) {
        ...
        for(int i = 0 ; i < sz ; i++) {
            ...
            if (p_name.equals(param)) return (p.SearchType(p_name)) ;
        }
        sz = locals.size() ;
        for(int i = 0 ; i < sz ; i++) {
    ...
    public SimpleExp SearchIdName(String p_name) {
        ...
        for( int i = 0 ; i < sz ; i++) {
            Variable p = (Variable)parameters.elementAt(i) ;
            String param = p.GetName() ;
            if  (p_name.equals(param)) return (new Temp(i+1)) ;
        }
        sz =  locals.size() ;
        for( int i = 0 ; i < sz ; i++) {
            Variable p = (Variable)locals.elementAt(i) ;
            String param = p.GetName() ;
            if (p_name.equals(param)) return p.GetTemp() ;
        }
    ...
    public boolean CheckType(MJType n_type) {
        if (!(n_type instanceof MJMethod)) return false ;
        else return true ;
    }
