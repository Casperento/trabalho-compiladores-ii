...
public class MJObj extends MJType {
    ...
    public String[] SortedFields() {
        ...
        if (f_names != null) return f_names ;
        if (extend_name != null) {
    ...
    public String[] SortedMethods() {
        ...
        if (m_names != null) return m_names ;
        if (extend_name != null) {
    ...
    public MJType SearchType(String field_name) {
        ...
        for(int i = 0 ; i < sz ; i++) {
            ...
            if (fn_name.equals(field_name)) return (f_var.SearchType(fn_name))  ;
        }
        if (extend_name != null) return class_extends.SearchType(field_name) ;
        return null ;
    }
    ...
    public MJType SearchMethod(String method_name) {
        ...
        for(int i = 0 ; i < sz ; i++) {
            ...
            if (m_name.equals(method_name)) return mj_met  ;
        }
        if (extend_name != null)
    ...
    public void Extentions(Vector v) {
        v.addElement(name) ;
        if (extend_name != null) class_extends.Extentions(v) ;
    }

    public boolean CheckType(MJType n_type) {
        int sz  = 0 ;
        if (!(n_type instanceof MJObj)) return false ;
        String has_ext = ((MJObj)n_type).GetExtenName();
        Vector names = new Vector();

        if (has_ext != null) ((MJObj)n_type).Extentions(names);
        else names.addElement(((MJObj)n_type).GetClassName());

        sz = names.size();
        for(int i = 0 ; i < sz ; i++) {
            String c_name = (String)names.elementAt(i);
            if (c_name.equals(name)) return true ;
        }
        return false  ;
    }
